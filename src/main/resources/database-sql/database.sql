...
CREATE TABLE `inspection_data` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT '主键',
  `HighT` double(3,2) DEFAULT NULL COMMENT '高区湿度摄氏度',
  `HighH` double(3,2) DEFAULT NULL COMMENT '高区湿度百分比',
  `MiddleT` double(3,2) DEFAULT NULL COMMENT '中区湿度摄氏度',
  `MiddleH` double(3,2) DEFAULT NULL COMMENT '中区湿度百分比',
  `LowT` double(3,2) DEFAULT NULL COMMENT '低区湿度摄氏度',
  `LowH` double(3,2) DEFAULT NULL COMMENT '低区湿度百分比',
  `ArmT` double(4,2) DEFAULT NULL COMMENT '线缆检测温度摄氏度',
  ...
  `Battery` double(3,2) DEFAULT NULL COMMENT '机器人电量',
